...
- **2026-05-31 Deprecated AI Tags Need Prompt, Input, Output, And Persistence Boundaries:** When removing structural tags from AI-generated thoughts, prompt edits alone are not enough; sanitize available tags, example tags, model output, manual writes, updates, and legacy custom-tag reads so stored data or model drift cannot reintroduce deprecated section labels.
- **2026-05-30 Title Capitalization Must Target The First Unicode Letter:** For user-entered titles, do not uppercase the first non-whitespace character; skip leading quotes, dashes, digits, emoji, and punctuation, then uppercase the first `\p{L}` match so values like `"впереди` become `"Впереди`.
- **2026-05-29 WAV Silence Must Match The Source Audio Format:** When concatenating Gemini TTS WAV output, generate inserted silence with the same sample rate and channel count as Gemini's raw PCM wrapper (24kHz mono). Mixing default 44.1kHz stereo silence into a 24kHz mono WAV concatenation keeps bytes silent but changes timing and makes the final file format contract false.
- **2026-05-29 Gemini TTS Free Tier Requires Section-Level Export, Not Current Small Chunks:** Gemini TTS may rank highly for quality, but My Preacher Helper's audio export bills quota per generated audio request. With active free-tier limits like 3 RPM and 10 RPD, the current small-chunk flow plus `TTS_CONCURRENCY = 6` will hit RPM/RPD quickly. A Gemini-specific mode should group by major sermon sections (intro/main/conclusion) and throttle requests, then split only sections that exceed TPM/input constraints.
...
- **2026-05-25 Vercel Build Runs `test:fast` As A Hard Deploy Gate:** `npm run build` runs `npm run test:fast && next build`, so ANY single failing test fails the Vercel build and silently blocks the deploy — a correct config/route fix never reaches prod. Before pushing to an auto-deploying branch, run the FULL `npm run test:fast` locally, not just targeted tests + `tsc`. (Cost me a wasted deploy: a route change broke its co-located route test that a truncated `find | head` had hidden.)
- **2026-05-25 Structured-Output Model Overrides Must Match The Active Provider:** `structuredOutput.ts` selects BOTH the client and the model from `AI_MODEL_TO_USE` (OPENAI/GEMINI). Any caller passing an explicit `model` (e.g. speech optimization forcing `OPENAI_OPTIMIZATION_MODEL`) hands an OpenAI model id to the Gemini client → OpenAI SDK `404 status code (no body)` ("model not found"), which the route wraps as 500. Gate model overrides on `getCurrentAIProvider()`; under non-OpenAI providers pass `undefined` to fall through to the provider's model.
- **2026-05-25 Long TTS/AI Routes Need `vercel.json` maxDuration + Parallel Chunks:** The audio `generate`/`optimize` routes were absent from `vercel.json`, so they ran at Vercel's ~10-15s default. One full TTS chunk (3800 chars, `gpt-4o-mini-tts`) ≈ 32s and serial generation ≈ 190s for a 6-chunk sermon → killed by the function limit (Hobby cap 60s). Add such routes to `vercel.json` maxDuration AND parallelize per-chunk work with a bounded pool that writes results by index (preserves order for concatenation).
- **2026-05-24 AI Generation GETs Must Bypass PWA API Caches:** Slow AI generation endpoints exposed as same-URL `GET` requests can be replayed from Workbox `NetworkFirst` API cache when the network timeout is shorter than model latency; add no-store headers, unique request ids, or prefer non-cacheable methods, and exclude those routes from service-worker API caching.
- **2026-05-24 next-pwa RuntimeCaching Entries Need Options With Fallbacks:** When `next-pwa` has `fallbacks` enabled, it assumes every `runtimeCaching` entry has an `options` object and reads `options.precacheFallback`; even `NetworkOnly` custom rules must include `options` (preferably a stable `cacheName`) or production `next build` can crash while generating the service worker.
- **2026-05-22 Breadcrumb Detail Resolvers Need Hook Boundaries:** Breadcrumbs should consume entity-detail hooks (`useXDetail`) instead of inline React Query/service calls, so tests can mock the resolver seam and each hook owns cache-first list lookup plus fallback fetch behavior.
- **2026-05-22 Study Detail Breadcrumbs Need Scoped Child Labels:** When adding child routes like `/studies/:id/edit`, keep the id segment resolved through the note title and handle the terminal child segment with a scoped i18n label so `/studies/:id` behavior stays unchanged.
- **2026-05-22 Edit Focus Chrome Belongs In The Private Layout Gate:** When a private route needs full-screen editing, hide shared nav/breadcrumb chrome with a route-derived layout focus flag and let the page own readable width/padding; do not solve it by mutating breadcrumb labels or unrelated view routes.
- **2026-05-22 Direct Create Shortcuts Need Canonical Empty Payloads:** When replacing placeholder `/new` routes, every hardcoded shortcut must call the create mutation with the same empty defaults and route to the real id; grep old `/new` links to zero while leaving legacy cleanup to its owning rail.
...
- **2026-05-20 Jest Route-Group Paths Need Escaped Parentheses:** When running targeted Jest tests under Next route groups like `app/(pages)/(private)`, escape parentheses in the CLI regex (`app/\\(pages\\)/\\(private\\)/...`) or use `--runTestsByPath`; quoted raw parentheses still mean regex groups and can return "No tests found."
- **2026-05-20 TipTap React NodeViews Should Own Async Resolver Subscriptions:** For inline atom chips whose label depends on async React cache, put the resolver hook inside the React NodeView so only chips subscribe/re-render; putting cache subscriptions in the parent editor can break unrelated editor tests and force providers where no chip exists.
- **2026-05-20 TipTap Wikilink Atoms Need Markdown Parser Priority:** For `tiptap-markdown` custom inline atoms, register a markdown-it `parse.setup` rule that emits temporary schema-recognizable HTML and give the atom parse rule priority plus empty content; otherwise StarterKit's normal link mark can consume the anchor before the node is created, breaking Markdown round-trip.
...
- **2026-05-19 Plan Volume Issues Need Paired Telemetry By User Prompt Hash:** When Short vs Detailed feels unchanged, compare `plan_point_content` records by the same `request.userMessage.hash` and outline point; if `style` reaches telemetry but deltas stay small, the issue is prompt-volume contrast (often Short is too permissive), not UI state loss.
...
- **2026-05-19 Removing Structural Tag Semantics Requires Grep-To-Zero Across Runtime, Tests, And Docs:** When deprecating automatic section/required tags, remove the whole propagation chain (`forceTag`, required tag fetching, telemetry fields, model fields, mocks, docs) and rewrite consistency checks so outline-linked thoughts without structural tags are valid while legacy structural-tag collisions/mismatches remain visible.
...
- **2026-05-10 Local PWA Must Avoid Half-Enabled SW State:** If localhost is still controlled by an old service worker while the current `next dev` run has `next-pwa` disabled, first-load failures become intermittent and misleading; make PWA-on/off commands explicit and use production preview for true PWA parity.
...
- **2026-05-03 AI Studio Daily Usage Can Hide RPM Bursts:** Gemini AI Studio usage charts aggregated by day can look low while 429s are still legitimate; rate limits are enforced by short windows such as RPM/TPM/RPD per project/model, so app-side parallel calls or retry storms need their own queue/backoff even on low daily totals.
