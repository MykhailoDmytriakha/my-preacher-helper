...
- **2026-03-17 Outdent Must Resolve By Structural Scope, Not Only List Depth:** In a mixed heading + untitled-nodeblock editor, the same outdent affordance should do a normal nested-list lift for child note content, but when the selection is on a branch-owned top-level bullet noteblock it should lift the whole nodeblock to the parent branch scope. Otherwise the UI exposes one button while secretly requiring two different mental models.
- **2026-03-17 Structural Outdent Must Cover Plain Paragraph Blocks Too:** Solving outdent only for top-level bullet noteblocks is insufficient. Users also treat plain paragraphs inside a leaf branch as movable structural blocks; when they outdent such a paragraph, the system should promote that paragraph into a parent-scope untitled nodeblock instead of disabling the affordance.
- **2026-03-17 Prefer Implicit Body Blocks Over Making Every Paragraph A Node:** When users need future promotion into untitled structural units, do not immediately turn every text leaf into a first-class node. Keep calm writing through an implicit body block inside each unit, and promote prose into an explicit untitled nodeblock only when it needs its own boundary, children, or independent manipulation.
- **2026-03-17 Hidden Heading Is Not A Substitute For Untitled Structural Units:** If users repeatedly want to insert nested thought blocks without inventing titles, ghost/demoted headings solve only visual friction, not the structural need. First classify whether the inserted unit is body prose, a secondary untitled structural block, or a true first-class branch before spending effort on heading-suppression UX.
...
- **2026-03-17 Question-Centered Synthesis Clusters Beat More Flat Counters:** Once workspace already has review lanes and synthesis queues, the next leverage step is to assemble each visible open question with linked support, contrast, and application branches in one derived card. That turns retrieval surfaces into an actual synthesis workflow without introducing a second truth model.
- **2026-03-17 Active Synthesis Must Ignore Unresolved Branches And Broken Relations:** Review/recovery surfaces may keep stale branch records visible, but synthesis queues must gate on `isResolved === true` and must not count unresolved relation items. Otherwise companion metadata overrides markdown truth and creates ghost work items.
- **2026-03-17 Synthesis Relation Context Should Come From Workspace Truth, Not Viewport Truth:** If synthesis explains whether a visible branch is supported or connected, its relation graph must be derived from the full workspace note set, not from the current filtered viewport. UI filters may narrow visible items, but they must not erase structural context.
- **2026-03-16 Synthesis Surfaces Should Use Visible Inventory But Broader Relation Context:** For workspace synthesis queues, branch membership should come from the currently visible note subset, but relation support counts should still come from the broader scoped relation graph. Otherwise filtered views lose the evidence/application context that explains why a visible branch matters.
- **2026-03-16 Shared Branch Inventory Prevents Workspace Surface Drift:** When a workspace already has review lanes and needs new synthesis lanes, first extract a shared hydrated branch inventory helper. Derive multiple surfaces from that one inventory instead of reparsing/hydrating notes independently for every lane family.
- **2026-03-16 Cross-Locale Link Semantics Need Canonical Keys Under Localized Labels:** If branch-link relations are authored through localized markdown link titles, the retrieval layer must normalize known aliases (EN/RU/UK, legacy raw i18n keys, etc.) into one canonical relation key before grouping, filtering, or counting. Otherwise relation lanes become language-fragmented and stop being a semantic layer.
- **2026-03-16 Relation Filters In Knowledge Workflows Should Match Both Participants:** When a workspace filters by a semantic relation like `supports`, showing only the source note is too weak for sensemaking. The filter should include resolved target notes too, or the review lanes and note list drift apart and the concept being supported disappears from the main retrieval surface.
- **2026-03-16 Scope-Relative Resolution Must Not Masquerade As Broken Anchors:** If a branch target is outside the currently scoped note subset but still exists in the workspace, do not classify it as an unresolved/broken anchor. Source-scope derivation and workspace-wide target resolution may need to be split so “anchor needs refresh” remains honest.
- **2026-03-16 Relation Semantics Can Mature As A Derived Layer First:** If markdown branch refs already contain `#branch=<id>` plus optional relation titles, the first high-leverage expansion is to derive note summaries, top relation labels, relation filters, and cross-note relation lanes from that seam. Do not invent a second persistence model just to make relations visible in the workspace.
- **2026-03-16 New Semantic Workspace Lenses Must Propagate Into Detail Navigation:** When a workspace gains a new semantic filter like `branchRelation`, the same query param must be supported in `filterAndSortStudyNotes`, workspace link builders, and detail-page prev/next navigation in the same change. Otherwise the new semantic layer collapses the moment the user drills into a note.
- **2026-03-17 Hash-Driven State Sync Must Use Guard Refs:** When an effect mounts a `hashchange` listener and manually triggers the handler on mount, it MUST use a `lastProcessedHashRef` or similar guard. Otherwise, any state update triggered by the handler (e.g. branch activation) that causes a re-render/re-mount will create an infinite loop of redundant syncs.
- **2026-03-17 Centralized Hash Parsing Utility Prevents Lint Drifts:** When specialized hash parsing utilities (`parseStudyNoteBranchIdFromHash`) exist, prefer them over manual inline parsing (e.g., `hash.substring(8)`). This ensures consistent decoding logic, prevents unused import warnings, and provides a stable anchor for future URI contract changes.
- **2026-03-17 Test Mocks Must Return Stable References for Dependencies:** Mocks for hooks like `useStudyNotes` or `useAuth` must return stable object references (defined outside the mock or via `mockReturnValue`). If the mock returns a new object on every call during a render cycle, it will trigger infinite re-initialization loops in components that use these values in `useEffect` or `useMemo` dependency arrays.
- **2026-03-16 Cross-Note Branch Review Surfaces Should Prefer Live Hydrated Titles:** If workspace review queues already have both note markdown and companion branch-state, do not render branch titles only from persisted `record.title`. Parse the live outline, hydrate identity, and use the live branch title when remap succeeds; keep persisted title only as fallback for unresolved anchors.
- **2026-03-16 Broken Branch Anchors Need Explicit User Feedback:** When a `#branch=<id>` deep link no longer maps back to a live branch after hydration, silent no-op destroys trust. Emit an explicit recovery signal in the UI (toast/banner) and log the miss for diagnostics.
...
- Untitled Nodeblock MVP: When a heading-first markdown outliner needs untitled nested structure without reopening branch identity/synthesis, the safest first implementation is to treat unordered list items inside branch bodies as secondary `untitled nodeblocks`. This gives nested structure and editor affordances without redefining the branch primitive.
- Outline Tab Priority Rule: In a mixed heading + untitled-nodeblock editor, `Tab` / `Shift-Tab` must resolve list-item nesting before heading promotion/demotion. If heading logic runs first, a list item can be accidentally converted into a heading and the secondary-structure model breaks.
- Horizontal Rule Can Serve As A Bounded Scope-Pop Seam: In a markdown-first outliner that keeps headings as first-class branches, a standalone horizontal rule line on a leaf branch can safely mean “exit this branch scope one level up” so trailing untitled content can become a sibling under the parent without inventing a new heading or switching to block JSON.
