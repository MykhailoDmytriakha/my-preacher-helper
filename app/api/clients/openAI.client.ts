...
Если транскрипция содержит ЛЮБЫЕ:
  • библейские цитаты (даже частичные)
  • упоминания ключевых тем проповеди
  • богословские рассуждения
  • отсылки к структуре проповеди (введение, заключение)
то считайте её АКТУАЛЬНОЙ (relevant: true).

ВАЖНО:
- При наличии любого из основных тегов ("Вступление", "Основная часть", "Заключение") relevant ДОЛЖЕН быть true
- Пример неверного определения: транскрипция с тегом "Заключение" и relevant: false - ЭТО НЕДОПУСТИМО
- Частичные цитаты ДОЛЖНЫ быть дополнены до полных в кавычках с указанием ссылки
- Даже если цитата упоминается в контексте будущих планов ("В заключении мы будем говорить..."), это считается relevant
- Если в tags присутствует хотя бы один основной тег, relevant ДОЛЖЕН быть true

1. Любое упоминание структуры проповеди (введение/заключение/основная часть) ДОЛЖНО:
   - Получать соответствующий структурный тег
   - Автоматически устанавливать relevant: true
   Пример: "Сейчас мы переходим к заключительной части" → tags: ["Заключение"], relevant: true
...
3. Даже косвенные ссылки на структуру проповеди считаются relevant:
   - "Возвращаясь к началу нашей беседы..." → relevant: true
   - "Как мы уже говорили в основной части..." → relevant: true
   - "В следующий раз мы рассмотрим..." → relevant: true

4. При наличии любого из этих элементов в тексте → relevant: true:
   - Библейские имена/титулы (апостол Павел, Христос, Сын Человеческий)
   - Богословские термины (искупление, освящение, благодать)
   - Упоминания книг Библии (даже без цитаты)
   - Ссылки на логику проповеди (аргумент, пример, вывод)`;
    ...
    // Validate tag/relevant consistency
    const structuralTags = ["Вступление", "Основная часть", "Заключение"];
    if (result.tags.some((tag: string) => structuralTags.includes(tag)) && !result.relevant) {
      log.warn('generateThought: Structural tag found with relevant=false, forcing to true');
