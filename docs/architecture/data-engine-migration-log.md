...
| ~~Mixed-mode collection reads~~ | `app/data-engine/activation.ts`, `collections.ts` | Fixed 2026-09-18: while legacy writers share a served collection the server says `legacyOpen` and the reader lists the whole collection on every synchronisation; closing the collection is a separate, last switch. **Cost:** the timed sweep ran every 15 s and, with QA tabs left open, exhausted the project's daily read allowance on 2026-09-23; it is now five minutes (`a0c28e53`), with opening a list, returning to the tab and reconnecting still sweeping at once. Keep the mixed phase short |
...
| Sermons | 33 | Core/preparation and scratch, creation/list/calendar, metadata/planned-date forms, manual thoughts, outline form, structure board and plan screens are integrated; every legacy SDK/HTTP writer refuses with `data-engine-required`; trusted server writers (transcription, insights, audio) write through `processCommand` (`serverEdit.server.ts`) | Real browser/device acceptance on Preview |
...
| Studies (notes + materials + share links) | 7 | Notes integrated 2026-09-23 (`useStudyNotesEngine`): list, create, edit with the baseline guard narrowed to the fields a save changes, delete (the server removes the note from its materials); the editor follows the row when clean and "take theirs" reads it; legacy writers guarded; note GET/DELETE and share-link creation answer an owner's tombstone as absent. Materials have no screen: their server routes stay legacy and refuse engine documents through `runLegacyTransaction`. Share links stay server-owned capability tokens (the engine registers `studyNoteShareLinks` read-only); the public read already hides a retired note | Live pass after the quota reset; Preview pass |
