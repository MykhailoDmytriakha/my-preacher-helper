...
jest.mock('@/utils/sermonSearch', () => ({
  matchesSermonQuery: jest.fn(() => true),
  tokenizeQuery: jest.fn(() => []),
}));
...
describe('AddSermonToSeriesModal', () => {
  ...
  it('opens create sermon modal when Create New Sermon button is clicked', async () => {
    ...
    expect(screen.getByTestId('preselected-series')).toHaveTextContent('series-123');
  });

  it('closes create sermon modal and adds new sermon to series', async () => {
    const user = userEvent.setup();
    renderWithProviders(<AddSermonToSeriesModal {...defaultProps} />);

    // Open create sermon modal
    const createButton = screen.getByRole('button', { name: /Создать новую проповедь/i });
    await user.click(createButton);

    // Create new sermon
    const createSermonButton = screen.getByTestId('create-sermon-button');
    await user.click(createSermonButton);

    expect(defaultProps.onAddSermons).toHaveBeenCalledWith(['new-sermon']);
  });

  it('renders close button in header', () => {
    renderWithProviders(<AddSermonToSeriesModal {...defaultProps} />);

    // Find close button by its icon (XMarkIcon)
    const closeButton = screen.getByRole('button', { name: '' }); // Empty aria-label, but has XMarkIcon
