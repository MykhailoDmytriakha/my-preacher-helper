...
describe('LanguageSwitcher Component', () => {
  ...
  test('menu buttons expose the selected language and preserve authenticated persistence', async () => {
    const { rerender } = render(<LanguageSwitcher variant="menu" />);
    const group = screen.getByRole('group', { name: 'navigation.language' });
    const buttons = within(group).getAllByRole('button');
    expect(buttons).toHaveLength(3);
    expect(buttons[0]).toHaveAttribute('aria-pressed', 'true');
    expect(screen.queryByRole('combobox')).not.toBeInTheDocument();
    for (const [index, language] of ['en', 'ru', 'uk'].entries()) {
      expect(buttons[index]).toHaveAttribute('type', 'button');
      fireEvent.click(buttons[index]);
      expect(mockChangeLanguage).toHaveBeenLastCalledWith(language);
      await waitFor(() => expect(mockUpdateUserLanguage).toHaveBeenLastCalledWith('test-user-id', language));
      (useTranslation as jest.Mock).mockReturnValue({
        t: (key: string) => key,
        i18n: { language, changeLanguage: mockChangeLanguage },
      });
      rerender(<LanguageSwitcher variant="menu" />);
      expect(within(group).getAllByRole('button', { pressed: true })).toEqual([buttons[index]]);
    }
  });
