...
export default function StudyNoteEditorPage() {
    ...
    useEffect(() => {
        ...
        if (isNew && !isInitialized) {
            // It's a new note, we create it immediately and redirect to its new ID
            createNote({
                title: '',
                content: '',
                tags: [],
                ...
                router.replace(`/studies/${newNoteId}`); // Swap URL without adding history
            }).catch(() => {
    ...
    }, [notesLoading, isNew, existingNote, isInitialized, createNote, uid, router, t]);


    // ─── AUTO-SAVE LOGIC ──────────────────────────────────────────────────
    // We only run auto-save if isInitialized is true.

    const saveChanges = useCallback(async () => {
        if (!noteId || isNew) return;

        // Compare current local state with what's in 'existingNote' to avoid empty/redundant saves
        if (existingNote) {
            const isUnchanged =
    ...
    const handleDelete = async () => {
        if (confirm(t('studiesWorkspace.deleteConfirm'))) {
            if (noteId && !isNew && uid) {
                ...
                try {
                    // We do this locally as a patch for missing endpoint
                ...
                } catch (e) { console.error('Error deleting share link', e); }
            }
            router.back();
        }
    };
            ...
            if (aiResult.scriptureRefs?.length > 0) {
                setScriptureRefs((prev) => {
                    const newRefs = aiResult.scriptureRefs.filter((nr: ScriptureReference) =>
                        !prev.some((er) => er.book === nr.book && er.chapter === nr.chapter && er.fromVerse === nr.fromVerse)
    ...
    const handleVoiceRecordingComplete = async (audioBlob: Blob) => {
        ...
        try {
            ...
            if (newText) setContent((prev) => (prev ? `${prev}\n\n${newText}` : newText));
        } catch {
