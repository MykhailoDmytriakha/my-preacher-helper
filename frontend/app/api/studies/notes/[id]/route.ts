...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const note = await studiesRepository.getNote(params.id);
    if (!note) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    if (note.userId !== userId) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
    return NextResponse.json(note);
  } catch (error) {
    console.error(`GET /api/studies/notes/${params.id} error`, error);
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const existing = await studiesRepository.getNote(params.id);
    ...
    const updated = await studiesRepository.updateNote(params.id, { ...updates, userId });
    return NextResponse.json(updated);
  } catch (error) {
    console.error(`PUT /api/studies/notes/${params.id} error`, error);
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await studiesRepository.deleteNote(params.id);
    return NextResponse.json({ success: true });
  } catch (error) {
    console.error(`DELETE /api/studies/notes/${params.id} error`, error);
