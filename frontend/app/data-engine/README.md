...
in closed checkpoints (`waitsForDecision`: a conflict, or a request the journal marks refused or
conflicted). Until the person decides, further one-shot changes to that document are refused
with `decision-required`, so nothing piles up behind the answer. `resolve(resource, choice)`
never chooses for the person: "mine" is allowed only when exactly one checkpoint waits and the
document still exists — the engine gives no order between several closed checkpoints of one
document (their `editGeneration`s restart and tie) — and "theirs" retires exactly the waiting
checkpoints, a refusal returning its draft to the confirmed copy. `EngineConflictBanner`,
mounted once in the private layout, shows one entry per document for the one-shot collections
with every changed field of every waiting draft, and offers only the choices `resolve` allows.
A per-document ordering of closed checkpoints (ownership across tabs, retirement of forks) is
an open engine-level item; see the migration log, 2026-09-23.
