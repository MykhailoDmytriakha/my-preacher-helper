...
| `plan_point_content` | `v12` | 2026-06-14 | v11 reshaped the layout to the approved reference plan: `turn` (route arrow) to the TOP under the anchor; `refs` moved from a top-level trailing block INTO each cue group (inline under that group's cues); sub-point headings as bold labels instead of `#### ` so depth caps at `###` and the Word export stays intact. v12 then (a) canonicalizes every arrow spelling (`->`, `—>`, `=>`, `→`) to a single `→` in the assembler, so the route line no longer mixes the assembler's `→` prefix with the model's `->`; and (b) puts a blank line between consecutive refs so each verse renders on its own line in both the plan UI and the Word export (previously `*a*\n*b*` collapsed into one run-together paragraph). | Review new `plan_point_content@v12` records after 2026-06-14 usage. Treat `v4`–`v11` as history/regression examples (v9 = structured cue-card, v10 = refs carry verse text, v11 = reshaped layout). |
...
| `plan_point_content` | `v12` | Generate a preacher cue sheet for one outline point or its sub-points: route arrow on top (arrows canonicalized to `→`), sub-points as bold labels, per-group Scripture refs rendered inline; detailed mode preserves more source-supported references, fragments, examples, and transitions. |
